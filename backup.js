...
const tokenize = async(prompt, options = {}, defaults = {}, fn = identity) => {
  let unique = new Set();
  ...
  while (i < input.length - 1) {
    ...
    if ((value === '$' || value === '#' || value === '{') && peek() === '{') {
      ...
      let item = tokens.find(item => item.name === token.key);
      token.field = fields.find(ch => ch.name === token.key);

      if (!item) {
        item = new Token(token);
        tokens.push(item);
      }

      item.lines.push(token.line);
      continue;
    }
  ...
  push({ type: 'eos', value: '' });
  return { input, tabstops, unique, keys, tokens };
};
...
module.exports = async prompt => {
  ...
  let { tabstops, tokens, keys } = await tokenize(prompt, options, defaults);
  ...
  let isValue = prompt.isValue.bind(prompt);

  return async(state = {}, submitted = false) => {
    let index = 0;

    state.required = required;
    state.tabstops = tabstops.filter(tab => tab.type === 'template');
    state.tokens = tokens;
    state.keys = keys;
    state.output = '';
    ...
    for (let token of tabstops) {
      ...
      if (token.type === 'template') {
        let item = tokens.find(ch => ch.name === key);

        if (options.required === true) {
          state.required.add(item.name);
        }

        let val = [item.input, state.values[item.value], item.value, value].find(isValue);
        ...
        if (index === state.pos) {
          if (before !== value) {
            ...
            value = prompt.styles.em(colors.unstyle(value));
          }
        }
