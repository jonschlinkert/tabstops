...
describe('.render', () => {
  it('should render a value with a variable passed to the constructor', () => {
    ...
    assert.equal(parser.render({ name: '' }), 'foo name bar');
  });

  it('should render a value with a tabstop', () => {
    let parser = new Parser('foo ${1} bar');
    assert.equal(parser.render(), 'foo  bar');
  });

  it('should render a value with a tabstop placeholder', () => {
    let parser = new Parser('foo ${1:Brian} bar');
    assert.equal(parser.render(), 'foo Brian bar');
  });
});
