...
const parse = (input, tabstops = new Map()) => {
  ...
  parser
    ...
    .handler('placeholder', node => {
      let [m, open, stop, variable, delim] = node.match;
      let block;

      if (stop && (!delim || delim === ':')) {
        block = new Tabstop({ nodes: [], match: node.match });
        block.tabstop = Number(stop);
      }

      if (stop && delim === '/') {
        block = new PlaceholderTransform();
        block.tabstop = Number(stop);
        parser.pushScope(new Scope({ type: block.type, blocks: [block] }));
      }

      if (!stop && delim === ':') {
        block = new Placeholder();
