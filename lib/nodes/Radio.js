...
class Radio extends Node {
  ...
  compile(options) {
    return data => {
      ...
      if (this.session && this.session.closed) {
        state.value = this.styles.unstyle(state.value);
      }
  ...
  get indicator() {
    let icon = this.icons[this.status];
    if (this.session && this.session.options.colors === false) {
      return this.enabled ? icon : this.symbols.cross;
    }
    if (!this.enabled && this.session && this.session.focused === this) {
      return this.styles.muted(icon);
    }
    return this.styles[this.status](icon);
  }
