...
const { define, trailingIndent } = require('../utils');
...
class Node {
  ...
  get indent() {
    if (this.type === 'text') {
      return trailingIndent(this.value);
    }
    let prev = this.prev;
    while (prev && prev.type !== 'text' && prev.prev) prev = prev.prev;
    return prev.indent;
  }
  ...
  get prev() {
    return this.siblings[this.index - 1] || (this.parent && this.parent.prev);
  }
  get next() {
    return this.siblings[this.index + 1] || (this.parent && this.parent.next);
  }
