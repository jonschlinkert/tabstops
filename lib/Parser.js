...
class Parser extends Events {
  ...
  field(node) {
    ...
    if (key !== null) {
      this.all.add(node);
      let nodes = this.fields.has(key) ? this.fields.get(key) : new Set();
      this.fields.set(key, nodes.add(node));
    }
  }
