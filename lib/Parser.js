...
class Parser extends Events {
  ...
  initialize() {
    ...
    const addTabstop = node => {
      let stop = this.stops.get(node.number) || [];
      this.stops.set(node.number, stop.concat(node));
      // console.log(this.stops)
    };

    const decorate = close => node => {
      node.data = this.data;
      node.fields = this.fields;
      node.tabstops = this.tabstops;
      node.stops = this.stops;

      if ((close === true || !node.nodes) && typeof node.number === 'number') {
        addTabstop(node);
      }
    };

    this.on('open', decorate());
    this.on('push', decorate());
    this.on('close', decorate(true));
  }
  ...
  compile(options) {
    let ast = this.parse();
    return ast.compile({ ...this.options, ...options });
  }
