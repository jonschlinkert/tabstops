...
module.exports = (state = {}, options = {}) => {
  const localize = (dateOptions = {}) => {
    let opts = { ...options, ...dateOptions };
  ...
  const variables = {
    TM_SELECTED_TEXT: (state.selection || ''),
    TM_CURRENT_LINE: (state.input || '').split('\n')[state.line] || '',
    TM_CURRENT_WORD: (state.input || ''),
    TM_LINE_INDEX: (state.line || 0),
    TM_LINE_NUMBER: (state.line || 0) + 1,
    TM_DIRECTORY: file.dirname || (file.path && path.dirname(file.path)) || options.cwd || process.cwd(),
    TM_PROJECT_DIRECTORY: options.cwd || (file.path && path.dirname(file.path)) || process.cwd(),
    TM_FILENAME: file.basename || (file.path && path.basename(file.path)),
    TM_FILEPATH: file.path,

    get TM_FULLNAME() {
      return options.fullname || 'Jon Schlinkert'; // <= remove me!!
    },
  ...
  for (let key of Object.keys(process.env)) {
    let prop = key.startsWith('ENV_') ? key : ('ENV_' + key);
    variables[prop] = process.env[key];
  }
