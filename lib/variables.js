...
module.exports = (data = {}, options = {}) => {
  ...
  //   let tm = key.startsWith('TM_');
  //   let prop = tm ? 'ENV_' + key.slice(3) : key;

  //   if (tm && !process.env[prop]) {
  //     Reflect.defineProperty(variables, prop, {
  //       get() {
...
// const variables = module.exports({ word: 'foo', clipboard: 'bar' }, { fullname: 'Jon Schlinkert' });

// let docs = [];
// for (let key of Object.keys(variables)) {
//   if (!key.startsWith('ENV_')) {
//     docs.push(`- \`${key}\`: e.g. \`${variables[key]}\``);
//   }
// }
